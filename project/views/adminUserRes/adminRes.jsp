					...
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach var="board" items="${userResList }">
								<!-- forEach 문 시작--------------------------------------------------------------------------------------------------------------------------------------->
								<tr style="cursor: pointer;" onclick="" class = "userResDetail">
			...
			<div class="modal-content">
				...
				<div style="text-align: right;">
					<a class="btn btn-secondary" role="button" onclick = "userResModalDelete()"
						...
						class="btn btn-secondary" href="#" role="button" onclick = "userResModalModfiy()"
...
function userResModalDelete(){ 
	var ures_seq= $('#ures_seq').text();
 	var deleteconfirm = confirm(ures_seq+'번 질문을 삭제하시겠습니까?');						    	
	if(!deleteconfirm){
		return false;
	}else{
	var seqData ={"ures_seq":ures_seq};
...
function userResModalModfiy(){  
 	var ures_seq= $('#ures_seq').text();	    	
	location.href = "./modify.do?ures_seq="+ures_seq;	 
};


$(document).ready(function(){       
    $(".userResDetail").on("click", function(){
    	 ...
    	 var res_birth = $(this).children("td:nth-child(15)").text();
    	 var res_email = $(this).children("td:nth-child(16)").text();
